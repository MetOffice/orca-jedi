...
void State::print(std::ostream & os) const {
  ...
  // field, so cache them and only recompute when the field contents change.
  if (!printCacheValid_) {
    ...
    for (atlas::Field field : stateFields_) {
      ...
      cache << std::string(8, ' ') << fieldName << ": " << std::setprecision(5)
            << norm_val << std::endl;
    }
    printCache_ = cache.str();
...
template<class T> double State::norm(const std::string & field_name) const {
  ...
  if (nonfinite_points > 0) {
    ...
    msg << classname() << "::norm '" << field_name << "' contains "
        ...
        << " out of " << static_cast<size_t>(total_points)
        << " model points; this indicates corrupt input data.";
    throw eckit::UserError(msg.str(), Here());
  }
