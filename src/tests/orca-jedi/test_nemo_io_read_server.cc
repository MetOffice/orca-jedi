...
CASE("test MPI distributed reads structured grid field array view") {
  for (auto & gridname : std::vector<std::string>{"amm1", "amm1r"}) {
    eckit::PathName test_data_path("../Data/" + gridname + "_nemo.nc");
    eckit::PathName grid_spec_path("../Data/" + gridname + "_atlas_grid_spec.yaml");
    ...
    for (std::string partitioner_name : partitioner_names) {
      ...
      SECTION(gridname + " " + partitioner_name + " get field _FillValue") {
        ...
        if (std::abs(default_missing_value - std::numeric_limits<float>::lowest()) >= 1e-6) {
          std::cout << "default_missing_value: " << missing_value << " == "
              ...
              << " diff " << std::abs(default_missing_value
                                      - std::numeric_limits<float>::lowest())
              << std::endl;
        }
      ...
      SECTION(gridname + " " + partitioner_name + " surface field") {
      ...
      SECTION(gridname + " " + partitioner_name + " volume field") {
