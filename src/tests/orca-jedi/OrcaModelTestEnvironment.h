...
struct Printer<atlas::PointLonLat> {
    static void print( std::ostream& out, const atlas::PointLonLat& v ) { out << std::fixed << std::setprecision( 12 ) << v; }
};
...
bool approx_eq( const atlas::Point2& v1, const atlas::Point2& v2, const double& t ) {
...
struct OrcaModelTestEnvironment {
    ...
    OrcaModelTestEnvironment( int argc, char* argv[] ) {
        ...
        atlas::Log::error().addTarget( new eckit::PrefixTarget( "[" + std::to_string( eckit::mpi::comm().rank() ) + "]" ) );
